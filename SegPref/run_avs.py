...
from datasets import AVS
from models import AVS_Model_Base
...
def run(model):
    train_dataset = AVS('train', args)
    val_dataset = AVS('val', args) 
    test_dataset = AVS('test', args)
    ...
    if args.val == 'val':
        ...
        val_loader = DataLoader(test_dataset, batch_size=4, shuffle=False, num_workers=0, pin_memory=False, collate_fn=collate_fn)
...
def collate_fn(batch):
    ...
    return uids, mask_recs, img_recs, image_sizes, audio_feats, text_feats

if __name__ == '__main__':
    print(vars(args))
    m2f_avs = AVS_Model_Base(cfgs=args)
